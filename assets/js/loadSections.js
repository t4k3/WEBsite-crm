...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!container || typeof v12Sections === "undefined") return;
  ...
  function renderSections(lang) {
    ...
    v12Sections.forEach((section) => {
      ...
      // Se la sezione ha un video lo mostra (autoplay muto in loop), altrimenti l'immagine.
      const media = section.video
        ? `<video src="${section.video}"${section.poster ? ` poster="${section.poster}"` : ""}
                 autoplay muted loop playsinline preload="metadata"
                 class="mx-auto mt-10 max-h-[54vh] w-auto rounded-3xl"></video>`
        : `<img src="${section.img}" alt="${text.title}"
                class="mx-auto mt-10 max-h-[54vh] w-auto object-contain rounded-3xl"
                loading="lazy"
